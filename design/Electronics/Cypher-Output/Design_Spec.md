...
| **Per-position LED indicator** | RGB LED per lens position, qty matching variant lens count; colour/brightness are **not** generated on this board - received entirely as a broadcast from the Cypher-Input board via `J4`/`J6` | D1-D26 / D1-D40 / D1-D12 - TBD RGB SMD (placeholder, same part as Cypher-Input, pending confirmation) |
...
LED current-limit resistors (R1-Rxx, one per colour channel per LED); the per-position select
...
`BRIGHTNESS_PWM_EN` (shared brightness gate, from Cypher-Input's own §6 Brightness Control). This
board's own per-position select MOSFETs (§4) gate these same 4 broadcast signals at each
lens position individually - the broadcast signals set *which colour, at what brightness*, while
...
| FR-CYPO-04 | Receive LED colour and brightness entirely as a broadcast from the Cypher-Input board - no local colour/brightness generation | `RED_DRIVE_N`/`GREEN_DRIVE_N`/`BLUE_DRIVE_N`/`BRIGHTNESS_PWM_EN` received on `J4`/`J6`; no local dial, oscillator, or I2C colour-config hardware on this board | §7 Interconnects; Colour / Brightness Reception (above) |
...
| DR-CYPO-06 | Per-position LED select topology | One N-channel MOSFET per lens position (SOT-23), gated directly by that position's decoded `plain-bits` line from the ENC module's `LBD_DEC` CPLD image; active-HIGH gate drive (CPLD output HIGH = MOSFET ON = that position's LED bank enabled, subject to the broadcast colour/brightness signals) | §4 LED Indicator Panel; BOM Q1-Q26 / Q1-Q40 / Q1-Q12 |
...
| DR-CYPO-11 | 3V3_ENIG entry decoupling bank | 5x 10uF X7R 50V 1206 at J4/J6 3V3_ENIG entry per `design/Standards/Global_Routing_Spec.md §3` Bulk Entry Bank Rule (single rail on this board) | §7 Power; BOM |
...
flowchart TD
  ...
  J4 -- "RED_DRIVE_N / GREEN_DRIVE_N / BLUE_DRIVE_N (received)" --> D
  J4 -- "BRIGHTNESS_PWM_EN (received)" --> D
  Q -- "per-position cathode gate" --> D
  D --> R --> GND[GND]
  J3 -- "3V3_ENIG" --> J2
...
- Q1-Qxx (one per lens position): gate driven directly by that position's `PB[n]` line.
- Active-HIGH gate drive: CPLD output HIGH -> MOSFET ON -> that position's LED lit (subject to
  the broadcast colour/brightness signals from Cypher-Input, received on `J4`/`J6` - see §1
  Colour / Brightness Reception). CPLD output LOW -> MOSFET OFF -> that position dark.
...
- **J4/J6 (left pair):** mates the Cypher Board's own `J5` template (`Cypher/Board_Layout.md §4`)
  - `3V3_ENIG` (pins 1-4), `5V_MAIN` (pins 5-8; not consumed by this board's own circuitry today -
  passthrough only, since this board's LED bank is driven by the broadcast colour/brightness
  signals rather than any local supply-dependent driver), GND (pins 9-12), `RED_DRIVE_N`
...
- **J5/J7 (right pair):** share the Cypher Board's board-agnostic HID Interconnect pin template
  ...
  `ENC_DATA[5:0]`, `ENC_ACTIVE_INPUT_N`. Pins 17-22 are GND on this template - `BOARD_ROLE_ID` is
  carried on the `J4`/`J6` left pair. Pins 27/28 (`I2C_SDA`/`I2C_SCL` on this template) are GND
  on this board - this board has no I2C bus connection (see §1). Pins 30/32 are unused (NC) on
...
- **Thermal:** No active cooling required. Q1-Qxx (2N7002K) dissipate well below 100mW combined,
  since only one position is ever lit at a time.
