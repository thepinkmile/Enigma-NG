...
interconnect, board-identification strap plus a shared non-cipher-key I2C expander) -
...
Colour and illumination values are stored and configured entirely on the User Settings Module and
delivered to this board on the common `J3` connector (`RED_DRIVE_N`/`GREEN_DRIVE_N`/
`BLUE_DRIVE_N`/`ILLUMINATION_DRIVE_N` - see `Design_Spec.md §5`/§6 and
