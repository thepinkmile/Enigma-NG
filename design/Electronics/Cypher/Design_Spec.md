...
> CM5 selects the active reflector map by writing the desired map index directly into the CPLD's
> UFM via the existing JTAG chain (in-system UFM write, using the same FT232H JTAG bridge already
> used for full CPLD programming) at configuration-apply time - no parallel GPIO bus or
> pull-down resistors are required. See `User_Settings_Module/Design_Spec.md §1` for the User
> Settings Module's own front-panel hardware. This mechanism's CPLD I/O headroom (see CPLD I/O
> Budget below) is used for the Cypher-Input/Cypher-Output `BOARD_ROLE_ID` compatibility
> comparator (see §3a).
...
`CFG_REFMAP[5:0]` is not used - the reflector map is selected via a JTAG UFM write instead (see
Configuration Bank 2 above). `BOARD_ROLE_ID_IN[3:0]`, `BOARD_ROLE_ID_OUT[3:0]`, and
`HID_VARIANT_ID[3:0]` occupy the resulting CPLD I/O. **All 76 user I/O pins are allocated.**
...
| Variant | Value | Notes |
| :--- | :--- | :--- |
| 26-Char Classic | `0b0001` | Characters only |
| 10-Numeric | `0b0010` | Numbers only |
| 64-Character (default) | `0b0111` | Characters + Numbers + Special |
| 64-Character (custom-support enabled) | `0b1111` | As above, plus Custom bit asserted via a user-accessible SPDT switch (Cypher-Output only - see `Cypher-Output/Design_Spec.md` once created) |
| Fully custom board | Any other 4-bit combination | Not an enumerated index - any bit combination not matching a defined variant is valid for a genuinely custom board's own capability declaration |

**Compatibility rule:** Cypher-Output is compatible with Cypher-Input if and only if every
capability bit set in the Input's ID is also set in the Output's ID - i.e.
`AND(BOARD_ROLE_ID_OUT, BOARD_ROLE_ID_IN) == BOARD_ROLE_ID_IN` (the Output's capabilities are a
superset of the Input's requirements). This single AND-based rule also correctly handles the
Custom bit with no special-casing: an Input with Custom set (bit 3 = 1) only matches an Output
that also has Custom asserted (e.g. the 64-Character Output's user-accessible switch bridged to
"on").

**Output:** `HID_VARIANT_ID[3:0]` = `BOARD_ROLE_ID_IN[3:0]` (mirrors the Input's own ID) when
compatible, or `0b0000` (the error/incompatible sentinel - not a valid ID for any real keyboard,
since a real board always asserts at least one capability bit) when the compatibility rule fails.
This drives an input pin bank on U7 (I2C GPIO expander, GPB[5:2]) for CM5
visibility.
...
| GPA | [6] | ENC_ACTIVE_INPUT_N | Bidirectional(Input) | Monitor: selected keyboard-source activity sideband (active-LOW) |
...
Handles CM5 virtual-key injection, mux-select, CPLD_RESET_N output, activity monitoring, and the
`BOARD_ROLE_ID` compatibility comparator result.
...
| GPB | [3] | `HID_VARIANT_ID[1]` | Bidirectional(Input) | From CPLD U1: resolved `BOARD_ROLE_ID` compatibility result bit 1 - see §3a |
| GPB | [4] | `HID_VARIANT_ID[2]` | Bidirectional(Input) | From CPLD U1: resolved `BOARD_ROLE_ID` compatibility result bit 2 - see §3a |
| GPB | [5] | `HID_VARIANT_ID[3]` | Bidirectional(Input) | From CPLD U1: resolved `BOARD_ROLE_ID` compatibility result bit 3 - see §3a |
| GPB | [6] | NC | Bidirectional | Not connected |
| GPB | [7] | NC | Output | MCP23017 silicon restriction: GPB[7] output-only (DS20001952D §1); NC |

> GPA[7] = CPLD_RESET_N (Output) — silicon-compatible. GPB[7] NC. GPB[5:2] carry
> `HID_VARIANT_ID[3:0]` (see §3a) - all bidirectional pins remain silicon-compatible.
...
- **Connector A (90-pin):** `plain-bits[63:0]` interface (ENC module to Cypher Board).
  ...
  defined in `Encoder_Module/Board_Layout.md §1a`.
- **Connector B (24-pin):** `cypher-bits[5:0]` (6) + JTAG (TCK, RST_N/CPLD_RESET_N, TMS, TDI,
  ...
  adjacent row position. Pin assignments defined in `Encoder_Module/Board_Layout.md §1b`.
- **Connector C (10-pin):** 3V3_ENIG power only (5x 3V3_ENIG + 5x GND). Pin assignments defined
  in `Encoder_Module/Board_Layout.md §1c`.
