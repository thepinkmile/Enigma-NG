...
2. **User requested canonical `.copilot/` session state in repo root** — so session state persists across sessions without UUID path issues.
   - Created `C:\_DATA_\Repositories\github\Enigma-NG\.copilot\` with `README.md`, `plan.md`, `checkpoints/index.md`, and both checkpoint files copied from user-profile session folder.
...
- Lives at `C:\_DATA_\Repositories\github\Enigma-NG\.copilot\` (gitignored)
